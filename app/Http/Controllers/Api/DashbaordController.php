...
{
    use ApiResponse;
    public function __construct(protected FirebaseService $firebase, protected UserService $userService)
    {
    ...
    {
        $currentUser = User::with('profile','viewCount')->findOrFail(auth()->id());

        // If caller specifically requests grouping by "iam_looking_for", return grouped results
        if ($request->query('show_looking_for') === 'true') {
            $grouped = $this->userService->getUsersGroupedByLookingFor($request, $currentUser);
            return $this->success('Users fetched successfully',$grouped);
        }

        $usersFormatted = $this->userService->getUsers($request, $currentUser);
